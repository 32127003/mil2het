...
class Walker:
        ...
        def run_exp(self, seed2weight, restart_prob, teleport_prob=0.5, normalize=False, node_list=[]):
                ...
                while (diff_norm > CONV_THRESHOLD):
                        ...
                        if arr_p.shape[1] >= MAX_ITER:
                                break
                print('%d iterated'%(arr_p.shape[1]))
                print('Final diff norm: %.8f'%(diff_norm))
        ...
        def _build_matrices(self, original_ppi, teleport_ppi, remove_nodes, constantWeight, absWeight, addBidirectionEdge):
                ...
                self.og_matrix = self._normalize_rows(og_not_normalized.T)
                ...
                self.tg_matrix = self._normalize_rows(tg_not_normalized.T)
...
def main_propagation(argv):
        ...
        for idx, wk in enumerate(lst_wk):#if there's multiple input graphs
                for j in range(arr_weights.shape[1]): #iterate (# samples) times
                        ...
                        lst_node_weight = wk.run_exp(seed2weight, args.restart_prob, teleport_prob=args.teleport_prob, normalize=str2bool(args.normalize))
                        set_tmpNodes=set()
                        ...
                        for node in set_nodes-set_tmpNodes:
                                ...
                                dic_node2weights[node].append(0.0)
        OF = open(args.o, "w")
        top_items = sorted(dic_node2weights.items(),
                   key=lambda kv: max(kv[1]) if kv[1] else float('-inf'),
                   reverse=True)[:]
        for node, weights in top_items:
