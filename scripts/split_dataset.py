...
def build_and_save_folds(
    config, random_seed: int , num_folds: int, out_dir: str, binary_positive_labels: Sequence[str], binary_negative_labels: Sequence[str]) -> None:
    if int(num_folds) < 3:
        raise ValueError(
    ...
    adata_path = os.path.join(config.adata_directory, config.dataset, f"{config.dataset}_data.h5ad")
    ...
    if str(config.dataset) == "asthma_ext":
        if not hasattr(config, "sample_column"):
            raise ValueError("sample_column is required for dataset='asthma_ext'.")
        sample_column_name = str(config.sample_column)
        if sample_column_name not in adata.obs.columns:
            raise ValueError(
                f"Missing sample column '{sample_column_name}' in adata.obs for dataset='asthma_ext'."
            )
    ...
    label_values_mapped, label_mapping_details = map_labels(
        ...
        binary_positive_labels=binary_positive_labels,
        binary_negative_labels=binary_negative_labels,
    )
    ...
    if str(config.dataset) == "asthma_ext":
        if sample_values is None:
            raise RuntimeError("Internal error: sample_values must be prepared for dataset='asthma_ext'.")
        ...
        print(
            f"asthma_ext sample grouping: total patient-samples={len(unique_patient_samples)} "
    ...
    for fold_index in range(num_folds):
        ...
        output_path = os.path.join(out_dir, f"{config.dataset}_idx_{fold_index}.pkl")
...
if __name__ == "__main__":
    ...
    # Run the whole split generation per patient_state.

    out_dir = os.path.join(str(config.splits_directory))
    os.makedirs(out_dir, exist_ok=True)
    ...
    build_and_save_folds(
        ...
        out_dir=out_dir,
        binary_positive_labels=config.binary_positive_label,
        binary_negative_labels=config.binary_negative_label,
    )
